 ...
 *
 * The accounts-on project is ALWAYS wired. When no local Supabase stack is
 * reachable its spec THROWS at load (fail loud, not skip) — a green run with
 * the geometry gate absent is never allowed. FEATURE_ACCOUNTS=true is set
 * ONLY on the throwaway server process Playwright spawns here; never on any
 * shared or hosted target.
 *
...
// The accounts-on project is ALWAYS declared, and its webServer ALWAYS wired
// with FEATURE_ACCOUNTS=true. When no local Supabase stack is reachable the
// spec throws at load (fail loud, not skip) and the server would fail to boot
// anyway — a green run with the geometry gate absent is never allowed.
projects.push({
  name: "accounts-on",
...
webServer.push({
  ...
  env: {
    ...
    SUPABASE_URL: supabase?.apiUrl ?? "http://127.0.0.1:54421",
    SUPABASE_PUBLISHABLE_KEY: supabase?.publishableKey ?? "e2e-accounts-no-stack",
    SUPABASE_SECRET_KEY: supabase?.secretKey ?? "e2e-accounts-no-stack",
